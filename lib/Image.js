 ...
 (function() {
    ...
    Image.prototype.extend({
        ...
        saveAs: function(filename) {
            if (!this.handle) {
                throw new Error('Image.resize - not constructed.');
            }
            if (!filename) {
                throw new Error('Image.resize - no filename argument provided.');
            }
            if (filename.indexOf('.') == -1) {
                throw new Error('Image.resize - invalid filename argument (no extension) ' + filename);
            }
            ...
            switch (extension) {
                ...
                case 'jpeg':
                    if (!gd.imageJpeg(this.handle, filename, -1)) {
                        throw new Error('Could not write ' + filename + '.  Error: ' + fs.error());
                    }
                    break;
                case 'png':
                ...
                case 'gif':
                    ...
                    break;
                default:
                    throw new Error('Could not write ' + filename + '. Files of .'+extension+' not supported.');
            }
        },
        dispose: function() {
            if (!this.handle) {
